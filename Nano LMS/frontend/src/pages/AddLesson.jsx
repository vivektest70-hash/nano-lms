...
export default function AddLesson() {
  ...
  const fetchCourse = async () => {
    try {
      ...
      // Check permissions
      if (!canManageLessons(courseData)) {
        toast.error('You do not have permission to add lessons to this course')
        navigate(`/courses/${courseId}`)
        return
      }

      // Set default order index to next available number
      const nextOrderIndex = (courseData.lessons?.length || 0) + 1
      setFormData(prev => ({
        ...prev,
  ...
  const canManageLessons = (courseData) => {
    if (isAdmin) return true
    if (isTrainer && courseData.instructor_id === user?.id) return true
    return false
  }
  ...
  if (!course) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-500">The course you're trying to add a lesson to doesn't exist.</p>
